...
ANN_Params={ 'hidden_layer_sizes': [(10,5), (10,5,3), (15,10,5,3)],
                     'alpha': [0.0001, 0.05],
                     'activation': ['tanh', 'relu']}
ANN = MLPClassifier()
model_ann=GridSearchCV(ANN,ANN_Params,n_jobs=-1,cv=5,scoring='f1_macro')
model_ann.fit(x_train,y_train)
print(f"Initial model best parameters {model_ann.best_params_}")
y_pseudo_pred = model_ann.predict(df_unlabeled)

y_pseudo_pred_ser = pd.Series(y_pseudo_pred)

y_concat_train = pd.concat([y_train,y_pseudo_pred_ser])
x_concat_train = pd.concat([x_train,df_unlabeled])

print("Number of items per class in the new Concatinated Dataset:")
print(y_concat_train.value_counts())
fig3, ax3 = plt.subplots()
ax3.pie(y_concat_train.value_counts(), autopct='%.2f', labels=[0,1])
#grouped_res=df_train['Class_ >50K'].value_counts().plot.pie(autopct='%.2f')
ax3.set_title("Dataset including Pseudo Labels")
print(" ")


mutual_information_results=ranking_features(x_concat_train,y_concat_train)
...
for model_name,mp in models_params.items():
    if mp['params']==None:
        clf=mp['model']
        clf.fit(x_concat_train,y_concat_train)
